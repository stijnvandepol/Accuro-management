<template>
  <div class="space-y-5 animate-slide-up">
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-3">
        <div class="relative">
          ...
          <input v-model="search" class="input pl-9 w-64" placeholder="Zoek op projectnaam..." />
        </div>
        ...
        <span class="text-xs font-mono text-gray-400 ml-2">{{ filteredProjects.length }} resultaten</span>
      </div>
    ...
    <div v-else class="card overflow-hidden light-table">
      <DataTable :value="filteredProjects" stripedRows paginator :rows="20" sortField="created_at" :sortOrder="-1"
        ...
        <Column field="status" header="Status" sortable style="width: 170px">
          <template #body="{ data }"><span :class="statusColor(data.status)" class="badge">{{ statusLabels[data.status] || data.status }}</span></template>
        </Column>
        <Column field="priority" header="Prio" sortable style="width: 100px">
          <template #body="{ data }">
            <div class="flex items-center gap-1.5"><div class="w-1.5 h-1.5 rounded-full" :class="statusDot(data.priority)"></div><span class="text-xs font-mono text-gray-500">{{ priorityLabels[data.priority] || data.priority }}</span></div>
          </template>
        </Column>
        <Column field="project_type" header="Type" sortable style="width: 140px">
          <template #body="{ data }"><span class="text-xs font-mono text-gray-500">{{ typeLabels[data.project_type] || data.project_type }}</span></template>
        </Column>
    ...
    <Dialog v-model:visible="showCreate" header="Nieuw project" modal :style="{ width: '560px' }">
      <form @submit.prevent="createProject" class="space-y-4">
        <div><label class="block text-xs font-medium text-gray-500 mb-1.5 uppercase tracking-wider">Projectnaam <span class="text-red-400">*</span></label><input v-model="form.name" class="input" required /></div>
...
const statusLabels: Record<string, string> = {
  ...
  TESTING: 'Testen',
  WAITING_FOR_CLIENT: 'Wachtend op klant',
  REVIEW: 'Review',
  COMPLETED: 'Afgerond',
  LIVE: 'Live',
